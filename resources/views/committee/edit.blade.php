...
@section('content')
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>Edit Committee Member: {{ $committee->name }}</h1>
            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('designation') is-invalid @enderror" id="designation" name="designation" value="{{ old('designation', $committee->designation) }}" required>
                        @error('designation')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="url" class="form-control @error('fb_link') is-invalid @enderror" id="fb_link" name="fb_link"
                                placeholder="https://facebook.com/username" value="{{ old('fb_link', $committee->fb_link) }}">
                            @error('fb_link')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="url" class="form-control @error('linkedin_link') is-invalid @enderror" id="linkedin_link" name="linkedin_link"
                                placeholder="https://linkedin.com/in/username" value="{{ old('linkedin_link', $committee->linkedin_link) }}">
                            @error('linkedin_link')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="email" class="form-control @error('gmail') is-invalid @enderror" id="gmail" name="gmail" 
                                placeholder="example@gmail.com" value="{{ old('gmail', $committee->gmail) }}" required>
                            @error('gmail')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('priority_level') is-invalid @enderror" id="priority_level" name="priority_level" 
                                value="{{ old('priority_level', $committee->priority_level) }}" min="0" required>
                ...
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-header bg-light">
                            <h6 class="mb-0">Member Image</h6>
                        </div>
                        <div class="card-body text-center">
                            <!-- Current Image Preview -->
                            @if($committee->image)
                                <div id="currentImageContainer" class="mb-3">
                                    <img src="{{ asset('storage/' . $committee->image) }}" class="img-fluid rounded border" style="max-width: 100%; max-height: 200px; object-fit: cover;">
                            ...
                            <!-- Default Preview when no new image -->
                            <div id="noImagePreview" class="mb-3" @if($committee->image) style="display: none;" @endif>
                            ...
                            <small class="text-muted d-block mt-2">
                                <i class="fas fa-info-circle"></i> Leave empty to keep current image. JPEG, PNG, JPG, GIF (Max: 2MB)
                            </small>
                        </div>
                    </div>
                </div>
        ...
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    if (noImagePreview) noImagePreview.style.display = 'none';
                ...
                if (noImagePreview && !currentImageContainer) {
                    noImagePreview.style.display = 'block';
                }
                if (currentImageContainer && !input.files.length) {
